...
db_exist_count=`ls ${REF}_db.* | wc -l`

if [[ ${db_exist_count} == 6 ]]; then
...
if [[ ${BLAST_TYPE} == "tblastx" ]]; then
      ...
      tblastx -query ${TARGET} \
        -db ${REF}_db \
        -out `basename ${TARGET}`_to_`basename ${REF}`.tblastx.txt.temp \
        -evalue ${evalue} \
        -num_threads ${threads} \
   ...
   echo -e "qseqid\tsseqid\tpident\tlength\tmismatch\tgapopen\tqlen\tqstart\tqend\tslen\tsstart\tsend\tevalue\tbitscore" | \
        cat - `basename ${TARGET}`_to_`basename ${REF}`.tblastx.txt.temp > `basename ${TARGET}`_to_`basename ${REF}`.tblastx.txt
   else
      echo "Using blastn."
      blastn -query ${TARGET} \
        -db ${REF}_db \
        -out `basename ${TARGET}`_to_`basename ${REF}`.blastn.txt.temp \
        -evalue ${evalue} \
        -num_threads ${threads} \
        -outfmt "6 qseqid sseqid pident length mismatch gapopen qlen qstart qend slen sstart send evalue bitscore"      
   echo -e "qseqid\tsseqid\tpident\tlength\tmismatch\tgapopen\tqlen\tqstart\tqend\tslen\tsstart\tsend\tevalue\tbitscore" | \
        cat - `basename ${TARGET}`_to_`basename ${REF}`.blastn.txt.temp > `basename ${TARGET}`_to_`basename ${REF}`.blastn.txt
   fi
